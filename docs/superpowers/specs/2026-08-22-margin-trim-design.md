...
WebSocket のメッセージ型は `progress` / `complete` / `error` の 3 種のみで、任意のログ行を
流す経路がない。新しい型を足すとフロントの型定義と表示パネルまで波及するため、**トリミングの
記録は既存の `logging` モジュールで `logs/` に出力する**にとどめる。

```
INFO 余白をトリミングしました: 2460x1375 → 1405x954 (残率 40%)
```

進捗表示は従来どおりファイル単位のカウンタのみとし、トリミングの有無は出力画像のサイズと
ファイル名の面積比で確認できる。
...
`ちとせよしの kiss you 3` の代表 8 枚で、`trim_margins` の ON/OFF を比較した結果:

| 項目 | 値 |
|------|-----|
| トリミング前の顔面積比 | 2.4% 〜 7.2% |
| トリミング後の顔面積比 | 3.0% 〜 17.6% |
| 残率（トリミング後 / 前の面積） | 18% 〜 40% |

上昇の倍率は素材によって 1.1 倍から 6.3 倍まで幅がある。余白の占める割合が
ページごとに異なるため、**旧閾値からの単純な換算式は存在しない**。

推奨する進め方は、まず閾値 3% 程度の低めで実行して出力ファイル名の `_N.Npct` の
分布を確認し、そこから目的の枚数になるよう調整することである。従来と同じ数値の
まま使うと、抽出される枚数が想定より増える点に注意すること。

余白のない素材（`小日向ゆり yuri mode Newtopia 348P`）では、`trim_margins` の
ON/OFF で出力画像サイズが完全に一致し、面積比も変化しないことを確認済み。
既存素材への影響はない。
